...
import { Search, Filter, MoreVertical, Plus, Server, Globe, DollarSign, CheckCircle, XCircle } from 'lucide-react';

const ApiProviderPage = () => {
  const providers = [
    { id: 1, name: 'Main SMM API', url: 'https://api.mainsmm.com/v2', balance: '$1,240.50', currency: 'USD', status: 'Active' },
    { id: 2, name: 'Backup Provider', url: 'https://backup-api.net/api/v2', balance: '$50.00', currency: 'USD', status: 'Active' },
    { id: 3, name: 'Premium Service Inc', url: 'https://premium-services.io/api', balance: '€120.20', currency: 'EUR', status: 'Inactive' },
    { id: 4, name: 'Cheap Followers API', url: 'https://cheapfollows.com/api', balance: '$0.05', currency: 'USD', status: 'Active' },
  ];
            ...
            <tbody className="divide-y divide-slate-100">
              {providers.map((provider) => (
                <tr key={provider.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-slate-700">{provider.balance}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      provider.status === 'Active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-800'
                    }`}>
