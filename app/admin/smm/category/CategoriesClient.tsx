...
type CategoryData = {
  ...
  name: string;
  status: string;
  // sort: not in schema yet, will omit or handle if needed
};

const CategoriesClient = ({ initialCategories }: { initialCategories: CategoryData[] }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-100">
              {categories.map((category) => (
                <tr key={category.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-slate-700">{category.id}</td> {/* Using ID as proxy for sort for now */}
                  <td className="px-6 py-4">
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
          <p className="text-sm text-slate-500">Showing <span className="font-medium text-slate-900">1</span> to <span className="font-medium text-slate-900">{categories.length}</span> of <span className="font-medium text-slate-900">{categories.length}</span> results</p>
