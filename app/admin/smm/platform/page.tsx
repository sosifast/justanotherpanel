...
import { Search, Filter, MoreVertical, Plus, Globe, CheckCircle, XCircle } from 'lucide-react';

const SmmPlatformPage = () => {
  const platforms = [
    { id: 1, name: 'Instagram', status: 'Active' },
    { id: 2, name: 'TikTok', status: 'Active' },
    { id: 3, name: 'YouTube', status: 'Active' },
    { id: 4, name: 'Facebook', status: 'Active' },
    { id: 5, name: 'Twitter', status: 'Inactive' },
    { id: 6, name: 'Spotify', status: 'Active' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <tr key={platform.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      platform.status === 'Active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-800'
                    }`}>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
          <p className="text-sm text-slate-500">Showing <span className="font-medium text-slate-900">1</span> to <span className="font-medium text-slate-900">6</span> of <span className="font-medium text-slate-900">6</span> results</p>
...
export default SmmPlatformPage;
